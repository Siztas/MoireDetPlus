...
class TripleBranchWithSpecificConv_MB_NoS(nn.Module):
    def __init__(self, model_config: dict):
        ...
        self.MB_TaylorFormer = MB_TaylorFormer()
        ...
        self.fea_reduce = nn.Conv2d(126, 24, 1)
    ...
    def forward(self, x, pure_moire=None):
        ...
        moire_fea = self.fea_reduce(moire_fea)

        trans_fea = self.MB_TaylorFormer(moire_fea).contiguous().view(B, -1, H, W)
...
class TripleBranchWithSpecificConv_MB(nn.Module):
    def __init__(self, model_config: dict):
        super().__init__()
        backbone = model_config['backbone']
        pretrained = model_config['pretrained']
        ...
        self.detail_conv = nn.Sequential(
            ...
            conv1x1(24, 48),
            nn.ReLU(inplace=True),
            conv1x1(48, moire_ouput_channel),
        )
        ...
        self.detail_bias = nn.Sequential(
            ...
            conv1x1(24, 48),
            nn.ReLU(inplace=True),
            conv1x1(48, self.ouput_channel * 3),
        )
        ...
        self.trans_fea_head = nn.Sequential(
            nn.ReLU(inplace=True),
            conv1x1(3,1)
        )
    ...
    def forward(self, x, pure_moire=None):
        ...
        moire_fea = self.fea_reduce(moire_fea)
        # moire_feas = moire_fea.split(1, dim=0)

        trans_fea = self.MB_TaylorFormer(moire_fea).contiguous().view(B, -1, H, W)

        # print(trans_fea.size())
        # print(moire_fea.size())
        ...
        _, _, Height, Width = moire_fea.size()
        ...
        detail_bias = self.detail_bias(moire_fea)
        # print(detail_bias.size())
        detail_bias = detail_bias.view(B, self.ouput_channel * 3, img_H, img_W)

        detail_fea = detail_fea + detail_bias

        # print(detail_fea.size())

        middle_fea = self.middle_fea(moire_fea)

        fea = torch.cat([detail_fea, middle_fea], dim=1)

        trans_fea = self.trans_fea_head(trans_fea)
        # print(fea.size())
        moire_density = self.moire_head(fea)
        # print(trans_fea.size())
        # print(moire_density.size())
        moire_density = moire_density + trans_fea
...
if __name__ == '__main__':
    ...
    x = torch.zeros(6, 3, 320, 320).to(device)
    ...
    model = TripleBranchWithSpecificConv_MB(model_config=model_config).to(device)
    y = model(x)
    print(y[0][0].size())
    # print(y.shape)
    # print(model)
