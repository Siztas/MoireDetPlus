...
class MB_TaylorFormer(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        inp_enc_level1_list = self.patch_embed_encoder_level1(inp_img)

        out_enc_level1 = self.encoder_level1(inp_enc_level1_list) + inp_img
...
if __name__ == "__main__":
    mb_taylorformer = MB_TaylorFormer()
    ...
    performer = Performer(
        ...
        depth=6,  # 设置模型深度
        heads=8,  # 设置头的数量
        dim_head=64,  # 设置每个头的维度
        local_attn_heads=0,  # 设置局部注意力头数量
        local_window_size=256
    )

    # 创建相同尺寸的输入数据
    input_tensor = torch.randn(1, 3, 256, 256)  # 假设输入为1x3x256x256的张量

    output_mb_taylorformer = mb_taylorformer(input_tensor)

    B, C, H, W = input_tensor.size()
    input_tensor_flattened = input_tensor.view(B, C * H * W)

    # 通过Performer传递调整后的输入数据
    output_performer = performer(input_tensor_flattened)

    # 打印输出尺寸
    print("MB-TaylorFormer Output Size:", output_mb_taylorformer.size())
    print("Performer Output Size:", output_performer.size())