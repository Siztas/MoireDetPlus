...
config = load_json('./script/predict_configs/predict_final_imgs_420.yaml')
...
def main(config):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, imgs_bak, moires, index_list) in tqdm(enumerate(train_loader)):
            ...
            for pred_moire, moire, index, img_bak, attention in zip(pred_moires, moires, index_list, imgs_bak,
                                                                    attentions):
                img_index += 1
